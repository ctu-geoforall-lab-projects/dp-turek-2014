...
Photogrammetry is used to produce a digital terrain models (DTM), a precise structure of objects or orthophotos. 
...
because the user loses a control over data.
...
It is focused on  processing of photos taken by rigorous 
...
DTM. The current processing chain is not able to meet up such a demand.
 The thesis analyzes the current state of orthorectification in GRASS,
...
Main goal of the chapter is to introduce reader into methods used by photogrammetry and computer vision
...
Least square methods are covered by the chapter because they  constitute core of BBA.
...
The image sensor is divided into cells measuring intensity of fallen light on their surface. The pixel value in final 
...
All light \term{rays} going from object points and falling on an image sensor are going through a same point in pinhole camera model.
...
where the object point is located. The system originates in projective center, Z coordinate 
 is coincident with projective ray and X, Y axes are parallel to x, y axes of a photo system.
...
Real positions of points and exterior orientation of cameras are expressed in \term{object coordinate system}. 
...
In the thesis, there are introduced and used methods leading to retrieval of the scene structure from the 
the set of photos, which are based on the simple principle of intersection of rays! Computer vision calls it structure from motion 
...
\begin{figure}[h]
    ...
    arising from perspective projection of the pinhole camera model. 
...
GRASS (Geographic Resources Analysis Support System) \footnote{http://grass.osgeo.org/} 
GIS is an open source GIS software 
capable of processing raster, vector and imagery data. 
GRASS belongs among one of 
the longest active software projects. Development of GRASS was begun by U.S. Army CERL  laboratory
...
The location comprises from \term{mapsets} used for organization 
...
on currently set region. Only exception are import modules ignoring it by default.
...
Besides military UAVs,  sector of civil drones is exponentially growing. It is very broad category ranging
...
even ordinary users are able to learn control them very quickly. Because of steeply falling prises of octocopters, it can be expected 
that this kind of drone will become commonly used for many civil purposes.

In photogrammetry, cheap civil UAVs can be successfully employed in mapping missions for creation of DTM/structure or orthophoto.
...
of corresponding photo points whose 3D object coordinates are obtained by intersection of rays (see \label{sec:ess_princip}). 
...
\item \evect{X} - vector of unknown parameters, whose values are searched to solve the system
\item \ematr{A} - it is matrix of a linear cost function  coefficients,  where row represents one measurement 
...
on a measurement. The random errors follows normal distribution represented by probability density function of this shape:
...
is that the mean of measurements performed by a more accurate 
instrument is statistically closer to the accurate value than the mean of same number of measurement performed
...
The overdetermined systems are solved in many fields dealing with measurements whose precision matters (e.g. 
...
Only if the measurements were perfect it would be possible to find such a parameters vector \evect{X} to 
solve a system of linear equations \eqref{eq:linear_eq}. 

Due to measurement imperfections it is not possible to find such a parameters of vector \evect{X} to make difference
\evect{v} between measured values and result of cost function using the parameters equal to zero:
...
The least squares method finds values of parameters vector \evect{X}, which 
...
According to calculus notation, the least squares method searches for global minimum of the square difference function. 
...
where:
\begin{itemize}
\item $\evect{X}_{0}$ is a point where a Taylor polynomial touches approximated function. 
...
rewritten into matrix form with formal substitution $\evect{L_{X_{0}}} = \efunc{f}_{i}(\evect{X}_{0})$:
\begin{equation}
\label{eq:expanded_taylor}
\efunc{T} ^{\efunc{f}_{i}, \evect{X}_{0}}_{1} (\evect{X}) = \evect{L_{X_{0}}} + \ematr{A}\evect{dx}
\end{equation} 

It should be noted that design matrix \ematr{A} contain partial derivatives of cost functions. 
...
The equation is slightly different compared to the linear least squares equation \eqref{eq:least_sq}
 Instead of parameters vector \evect{X} there is used 
...
This is caused by run-off of minimized function, which can have more points with zero derivative
...
and vector of parameters \evect{X} is non-random, then
...
\escal{const} is chosen arbitrary. It can be selected as a priori standard deviation of unit weight $\sigma_{0}$. 
...
some criteria allowing to assess that global minimum was reached. 
...
lest squares has enough iteration to converge to
global minimum. 
...
inverse. Another option is adding inner constraints to the design matrix.
...
Exterior and interior orientation together describes relation between an 3D object point and
its corresponding 2D photo point.
...
\begin{itemize}
  ...
  \item Principal point coordinates $(\escal{x}^{ph}_{pp}, \escal{y}^{ph}_{pp})$ - In ideal case position of principal point would be exactly in the middle 
...
tie points because 
parameters of interior orientation are obtained by a camera calibration procedure.
...
they do not require knowledge of object coordinates of photo points.
...
\caption{Difference of individual Euler angles ($\phi$, $\omega$, $\kappa$) between 
calculated angle and corresponding  angle from Bingo-F adjustment.
\label{table:ph_angles_conv}
\term{Bef. I.} column represents differences before BBA iteration was begun.
\term{I. n} column represents differences after \term{n}th BBA iteration.
The differences are expressed in degrees. Last two rows representing average and median 
are computed from absolute values of the differences. Average and maximum values 
are calculated from absolute values.}
\\\hline
