...
The core of photogrammetry is quickly moving  from  hardware towards software. In past, it was necessary to use special 
...
main branches: a proprietary software and free software. Most of a proprietary software is not available in 
...
All light \term{rays} going from object points and falling on an image sensor are going through the same point in pinhole camera model.
...
In the thesis, there are introduced and used methods leading to retrieval of a scene structure from the 
the set of photos which are based on such a simple principle of the ray intersection! Computer vision calls it structure from motion 
...
\begin{figure}[h]
    ...
    \caption{In the left picture, there is clearly visible effect of elevation differences 
    caused by perspective projection of pinhole camera model. 
    In the right picture, there is depicted orthogonal projection of orthophoto. (based on \cite{pavelka2004foto20}[p. 151])}
    \label{fig:ortho}
\end{figure}
...
even ordinary users are able to learn control them very quickly. Because of steeply falling prices of octocopters, it can be expected 
...
\item \evect{X} - vector of unknown parameters whose values are searched to solve \eqref{eq:linear_eq},
\item \ematr{A} - it is a matrix of a linear cost functions coefficients  where row represents one measurement
		  ...
		  Least square method calls it \term{design matrix}. 
\end{itemize}
...
\begin{itemize}
\item \term{gross error} - it is caused by human factor or fault of instrument. If measurement is repeated,
...
kind of error.
\item \term{systematic error} - if measurement is repeated by same instrument,
it has same effect (value) on measurement.  Usually effect of this error can be 
...
instrument is statistically closer to the accurate value than the mean of same number of measurements performed
...
The equation minimized by least squares is extended by weight matrix into following form:
\begin{equation}
...
\item $\evect{X}_{0}$ is a point where a Taylor polynomial touches approximated function. 
      ...
      is a line which is a tangent of function at a point $X_{0}$ in \eucl{2}, tangent plane in \eucl{3} and  
       tangent hyperplane in  \eucl{n}.
\item \evect{X} is a point where a Taylor polynomial returns approximate value of cost function $\efunc{f}_{i}$.
\end{itemize}
...
It can be rewritten into matrix form with formal substitution $\evect{L_{X_{0}}} = \efunc{f}_{i}(\evect{X}_{0})$:
\begin{equation}
\label{eq:expanded_taylor}
\efunc{T} ^{\efunc{f}_{i}, \evect{X}_{0}}_{1} (\evect{X}) = \evect{L_{X_{0}}} + \ematr{A}\evect{dx}
\end{equation} 

It should be noted that design matrix \ematr{A} contains partial derivatives of cost functions. 
...
If the differences vector from previous equation is substituted into least squares minimization criterion:
\begin{equation}
\label{eq:min_cond}
\evect{v}^{T}  \ematr{P} \evect{v} = min
\end{equation}
...
The equation is slightly different compared to the linear least squares equation \eqref{eq:least_sq}
 Instead of parameters vector \evect{X}, result of the equation is 
...
In new iteration, adjusted parameters vector \evect{X}
from previous iteration is used to eventually approach global minimum of the cost function. 
...
distributed (see Figure \ref{fig:norm_dist}) then values obtained from cost functions using  computed parameters lies in maximum of the density
...
lest squares method has enough iteration to converge to
global minimum. 
...
\begin{figure}[h]
    ...
    exterior orientation. \escal{x}, \escal{y}, \escal{z} represents axes of photogrammetric camera system and 
     \escal{X}, \escal{Y}, \escal{Z} are axes of an arbitrary object system.}
    \label{fig:ph_model}
\end{figure}
...
\begin{itemize}
  ...
  \item Principal point coordinates $(\escal{x}^{ph}_{pp}, \escal{y}^{ph}_{pp})$ - In ideal case position of principal point would be exactly in the middle 
	of a photo.  In most cases, there is some shift of principal point from a photo center due to
...
used model for distortion is Browns model \cite{brown1966distortion}.
...
because usually an object coordinate system is not coincident with camera coordinate system (see Figure \ref{fig:ph_model}). 
...
It is impossible to express by Cartesian coordinates with finite numbers:
...
Similarly a line in \eucl{2}  can be derived as cross product of two points in homogeneous form:
...
\item \ematr{[\ematr{R}|\evect{t}]} matrix contains rotation matrix\ematr{R} and vector $\evect{t}$.
	      They transform point from object coordinate system 
