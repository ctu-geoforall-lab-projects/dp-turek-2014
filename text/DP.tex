...
The GIS is also used for analyzing spatial phenomenons, which
...
%%% ST: prepsano
The epipolar line equation can be rewritten by the matrix notation of cross product as:
\begin{equation}
...
outliers by giving e.g. some treshold of standard deviation of scale computed 
...
effect.
Another way how to improve final accuracy of BBA is skipping points, 
standard deviation of which is higher than some threshold. The standard deviation criterion can be assessed 
...
where big matrices are solved. 
The size of the matrix is directly connected to the number of adjusted parameters.
...
as last step when the reimplemented part is tested, optimized and works well. Otherwise it would be waste of time and energy, if the 
...
There is trend to avoid adding new dependencies into GRASS because it already relies on many software libraries
 ...
 Another consequence of lot of dependencies is high  
...
Another area for improvement not being explored studied of the thesis is impact of accuracie
...
by elegancy of both fields consisting in a way how they build up incredibly powerful methods based on simple principles.  
...
Another great thing about the topic is that it is clear if it works because
 ...
 The developed solution was tested on the test case comprising of small set of photo capturing a scene.
 It was demonstrated that 
 the scene was successfully retrieved. The argument is supported by the fact that 
 that the retrieved results are comparable to the results of state of the art Bingo-F software.
 However  the solution can be considered merely as prove of concept running only on one test case presented in the thesis. 
 The solution is miles away from being used in practice. 
 The thesis also analyzes current state of orthorectificaion in GRASS and roughly outlines the way of its modification,
