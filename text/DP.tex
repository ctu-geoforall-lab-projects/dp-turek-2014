...
allowing to use the GCP visible only in a single photo. In this case, the object coordinates 
...
In the first least squares iteration, the initial values of adjusted parameters are put into parameters vector $\evect{X}$
 representing tangent point of Taylor polynomial to the costs functions.  The other non-adjusted 
...
The vectors  $\evect{L}$ and  $\evect{X}$ give structure to the design matrix $\evect{A}$ and
...
Position in a vector $\evect{L}_{X0}$ is connected in the same way to a vector $\evect{L}$ as rows of the design matrix.
...
are obtained. Another iteration can be  performed by the same procedure  with adjusted parameters vector $\evect{X}$
