...
At first point in the first photo system is transformed into the camera coordinate system of the first photo. 
Result of the transformation is point at the infinity, which describes ray represented by photo point. 
After that the ray is transformed into camera coordinate system of the second camera. This is done by so called essential matrix:
\begin{equation}
	 \ematr{E}  = [\evect{t}]_{\times} \ematr{R}
\end{equation}


As the last step, the ray is transformed into photo system of the second photo as epipolar line.
...
Triangulation methods deals with retrieval of object point coordinates
...
\item \evect{X} is object point,
\item \evect{C} is projective center of the shared photo, which belongs to both relative orientations.
\end{itemize}

The equation gets simplified if the relative orientation coordinate system coincide to the camera system of the shared photo, thus 
coordinates of the shared camera projective center is zero in both systems: 
\begin{equation}
...
Group of photos, which can be transformed into common system, is composed of the relative orientation photo pairs
...
The other method can be considered a extension of the first one because initially it generates DTM then it recovers orthophoto 
