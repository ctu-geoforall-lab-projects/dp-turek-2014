...
def RelativeOrientation(in_dt, cam_m, distor, rel_or_phs):
    ph_ids_to_ro_id = {}
    ros = []
...
def computeRelativeOrientation(tie_pts, cam_m, distor):
    ...
    err1 = computeError(tp2, pts_sps, R, t, cam_m, distor)
    pm1 = np.eye(3, 4)
    ...
    return np.hstack((pts_sps, np.expand_dims(err, axis=0).T)), P, p1, p2
